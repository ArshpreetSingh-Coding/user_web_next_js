...
const DestinationField = memo(({ value, onChange, className, variant }: DestinationFieldProps) => {
  ...
  return (
    <TimelineRow
      icon={<Navigation className={`w-4 h-4 sm:w-5 sm:h-5 text-primary fill-primary ${variant === "outline" ? "text-black! fill-black!" : ""}`} />}
    ...
    >
      ...
      <Input
        ...
        className={`h-10 lg:h-11 px-4 bg-white text-gray-900 placeholder:text-gray-400 border-0 focus-visible:ring-2 focus-visible:ring-white/50 text-sm ${variant === "outline" ? "border border-border" : ""}`}
      />
    </TimelineRow>
  );
});
